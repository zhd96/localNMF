...
function [J,normH,U] = FastSepNMF_dz(M,r,normalize,th) 
...
while i <= r && max(normH./normM2) > th
    ...
    normH = sqrt(normM);
    %H = M(:,J)\M;
    %normH = sqrt(sum((M - M(:,J)*H).^2));
    %normH = max(abs(M - M(:,J)*H));
    i = i + 1;
    %if (i==3)
    %    disp("haha")
    %end
