...
def prepare_iteration(Yd, connect_mat_1, permute_col, unique_pix, pure_pix, U_mat, V_mat, more=False):
    """
    ...
    if more:
        start = time.time();
        U, S, V = np.linalg.svd(Yd.reshape(np.prod(dims[:-1]),-1, order="F"), full_matrices=False);
        rank = sum(S > 1e-1);
        U = U[:,:rank]*S[:rank];
        V = V[:rank, :].T;
        ...
        return U_mat, V_mat, U, V, normalize_factor, brightness_rank, pure_pix#, corr_img_all_r
    else:
...
def make_mask(corr_img_all_r, corr, mask_a, num_plane=1,times=10):
    s = np.ones([3,3]);
        ...
        for kk in range(num_plane):
            ...
            while jj<=times:
                ...
                if num_features>1:
                    ...
                    if labeled_array.sum()/unit_length < 0.2 or corr==1 or corr==0:
                        break;
                    else:
                        print("corr too low!")
...
def make_mask1(corr_img_all_r, corr, a, num_plane=1):
    """
...
def update_AC_l2(U, V, normalize_factor, a, c, patch_size, corr_th_fix,
                 maxiter=50, tol=1e-8, update_after=None,merge_corr_thr=0.5,merge_overlap_thr=0.7, num_plane=1, plot_en=False,hals=False):
    """
    ...
    mask_a = make_mask(corr_img_all_r, corr_th_fix, mask_a, num_plane);
...
def update_AC_bg_l2(U, V, normalize_factor, a, c, patch_size, corr_th_fix,
                    maxiter=50, tol=1e-8, update_after=None,merge_corr_thr=0.5,merge_overlap_thr=0.7, num_plane=1, plot_en=False, ff=None,hals=False):
    """
        update spatial and temporal components using correlation image as constraints, with L2 loss
...
def hillman_data_pipeline(Yd, cut_off_point=[0.9,0.8], length_cut=[15,10], th=[2,1], pass_num=2, residual_cut = 0.43,
                          corr_th_fix=0.4, merge_corr_thr=0.5, merge_overlap_thr=0.8, num_plane=1, patch_size=[100,100],
                          plot_en=False, TF=False, fudge_factor=1, text=True, bg=False, max_iter=21, max_iter_fin=41,hals=False): #length_cut2=10, residual_cut2 = 0.6,
    ...
    print(time.time()-start);
        ...
        for kk in range(num_patch):
            ...
            unique_pix_temp, M = search_superpixel_in_range(connect_mat_1[up:down,left:right], permute_col, c_ini);
...
if plot_en:
    pure_superpixel_compare_plot(connect_mat_1, unique_pix, pure_pix, text);
        ...
        if ii > 0:
            ...
            a, c, U, V, normalize_factor, brightness_rank, pure_pix = prepare_iteration(Yd, connect_mat_1, permute_col, unique_pix, pure_pix, a_ini, c_ini, more=True);
    ...
    maxiter=max_iter;
        ...
        if bg:
            a, c, b, fb, ff, res, corr_img_all_r, num_list = update_AC_bg_l2(U, V, normalize_factor, a, c, dims,
                                                                             corr_th_fix, maxiter=maxiter, tol=1e-8, update_after=2,
                                                                             merge_corr_thr=merge_corr_thr,merge_overlap_thr=merge_overlap_thr, num_plane=num_plane, plot_en=plot_en, ff=ff,hals=hals);
    
        else:
            a, c, b, fb, ff, res, corr_img_all_r, num_list = update_AC_l2(U, V, normalize_factor, a, c, dims,
                                                                          corr_th_fix, maxiter=maxiter, tol=1e-8, update_after=2,
                                                                          merge_corr_thr=merge_corr_thr,merge_overlap_thr=merge_overlap_thr, num_plane=num_plane, plot_en=plot_en,hals=hals);
        #if plot_en:
...
if plot_en:
    ...
    if pass_num > 1:
        ...
        ax1.title.set_fontweight("bold")
        plt.show();
    fin_rlt = {'a':a, 'c':c, 'c_tf':c_tf, 'b':b, "fb":fb, "ff":ff, 'res':res, 'corr_img_all_r':corr_img_all_r, 'num_list':num_list, "U":U, "V":V};
    if pass_num > 1:
        return {'rlt':rlt, 'fin_rlt':fin_rlt, "superpixel_rlt":superpixel_rlt}
else:
    return {'fin_rlt':fin_rlt, "superpixel_rlt":superpixel_rlt}

def hillman_data_pipeline_2pass(Yd, a, c, b, cut_off_point=[0.9,0.8], length_cut=[15,10], th=[2,1], pass_num=2, residual_cut = 0.43,
                                corr_th_fix=0.4, merge_corr_thr=0.5, merge_overlap_thr=0.8, num_plane=1, patch_size=[100,100],
                                plot_en=False, TF=False, fudge_factor=1, text=True): #length_cut2=10, residual_cut2 = 0.6,
    #merge_corr_thr_fin = 0.8, merge_overlap_thr_fin = 0.8, plot_en=False):
    ...
    start = time.time();
    print("rank 1 svd!")
    c_ini, a_ini, _ = spatial_temporal_ini(Yt, comps, idx, length_cut[ii], maxiter=5, whole_data=True, method="svd");
    print(time.time()-start);
    unique_pix = np.asarray(np.sort(np.unique(connect_mat_1))[1:]); #search_superpixel_in_range(connect_mat_1, permute_col, V_mat);
    #unique_pix = np.asarray(unique_pix);
    pure_pix = [];
...
if plot_en:
    ...
    print("prepare iteration!")
    if ii > 0:
        a_ini, c_ini, U, V, normalize_factor, brightness_rank, pure_pix = prepare_iteration(Yd, connect_mat_1, permute_col, unique_pix, pure_pix, a_ini, c_ini, more=True);
        a = np.hstack((a, a_ini));
        c = np.hstack((c, c_ini));
    ...
    a, c, b, res, corr_img_all_r, num_list = update_AC_l2(U, V, normalize_factor, a, c, dims,
                                                          corr_th_fix, maxiter=maxiter, tol=1e-8, update_after=2,
                                                          merge_corr_thr=merge_corr_thr,merge_overlap_thr=merge_overlap_thr, num_plane=num_plane, plot_en=plot_en);
#if plot_en:
#    temporal_comp_plot(c,num_list,ini = False);
#spatial_comp_plot(a, corr_img_all_r, ini=False);
superpixel_rlt.append({'connect_mat_1':connect_mat_1, 'pure_pix':pure_pix, 'brightness_rank':brightness_rank});
np.save("full_2pass_a.npy", {'a':a});
np.save("full_2pass_c.npy", {'c':c});
np.save("full_2pass_b.npy", {'b':b});
np.save("full_2pass_res.npy", {'res':res});
#np.save("full_2pass_corr_img_all_r.npy", {'corr_img_all_r':corr_img_all_r});
np.save("full_2pass_num_list.npy", {'num_list':num_list});
    np.save("full_2pass_superpixel.npy", {"superpixel_rlt":superpixel_rlt});
    ...
    #    if plot_en:
    #        temporal_comp_plot(c, num_list, ini = False);
    np.save("full_2pass_c_tf.npy", {'c_tf':c_tf});
    fin_rlt = {'a':a, 'c':c, 'c_tf':c_tf, 'b':b, 'res':res, 'num_list':num_list};
...
def merge_patch_boundary(rlts, dims, patch_height, patch_width, patch_ref_mat, temp_cor=0.5):
    ########## combine all the components together and find all the boundary components ################
    b_zero = np.zeros((dims[0], dims[1], 1));
    for kk in range(np.prod(patch_ref_mat.shape)):
        ii = np.where(patch_ref_mat == kk)[0][0];
...
############## find those boundary components spatially adjacent/overlapped #######################
...
#                    cut_off_point2=0.95, length_cut2=10,cut_off_point3=[0.9,0.8], plot_en=False, more_pass=0, TF=False, fudge_factor=1): #length_cut2=10, residual_cut2 = 0.6,
#                    #merge_corr_thr_fin = 0.8, merge_overlap_thr_fin = 0.8, plot_en=False):
#
...
#
#    Cnt = local_correlations_fft(Yt);
#    V_mat, U_mat, B_mat = spatial_temporal_ini(Yt, comps, idx, length_cut, maxiter=5, whole_data=True, method="svd");
...
#    a, c, b, res, corr_img_all_r, num_list = update_AC_l2(U, V, normalize_factor, a_ini, c_ini, patch_size,
...
#        spatial_comp_plot(a, corr_img_all_r, num_list, ini=False);
#    ############# second pass #################
#    if refine:
...
#        if num_plane > 1:
#            print("3d data!");
#            connect_mat_1_res, idx_res, comps_res, permute_col_res = find_superpixel_3d(Yt_res,num_plane,cut_off_point2,length_cut2,eight_neighbours=True);
#        else:
#            connect_mat_1_res, idx_res, comps_res, permute_col_res = find_superpixel(Yt_res,cut_off_point2,length_cut2,eight_neighbours=True);
#        Cnt_res = local_correlations_fft(Yt_res);
#        V_mat_res, U_mat_res, B_mat_res = spatial_temporal_ini(Yt_res, comps_res, idx_res, length_cut2, maxiter=5, whole_data=True, method="svd");
...
#        a_res, c_res, b_res, res_res, corr_img_all_r_res = update_AC_l2(U_res, V_res, normalize_factor_res, a_ini_res, c_ini_res, patch_size,
#                                                            corr_th_fix, maxiter=50, tol=1e-8, update_after=5,
#                                                            merge_corr_thr=merge_corr_thr,merge_overlap_thr=merge_overlap_thr, num_plane=num_plane);
#        #if plot_en:
...
#            Cnt_res = local_correlations_fft(Yt_res);
#            V_mat_res, U_mat_res, B_mat_res = spatial_temporal_ini(Yt_res, comps_res, idx_res, length_cut, maxiter=5, whole_data=True, method="svd");
#            unique_pix_res, M_res = search_superpixel_in_range(connect_mat_1_res, permute_col_res, V_mat_res);
#            pure_pixels_res, coef_res, coef_rank_res = fast_sep_nmf(M_res, M_res.shape[1], residual_cut);
#            if plot_en:
#                pure_superpixel_compare_plot(connect_mat_1_res, unique_pix_res, pure_pixels_res);
...
#
#        if plot_en:
#            spatial_sum_plot(a, a_ini_fin, patch_size);
#
#        print("final update!");
#        #return {'a_fin':a_ini_fin, 'c_fin':c_ini_fin, 'y0':y0}
#        a_fin, c_fin, b_fin, res_fin, corr_img_all_r_fin = update_AC_l2(U, V, normalize_factor, a_ini_fin, c_ini_fin, patch_size,
#                                                            corr_th_fix, maxiter=40, tol=1e-8, update_after=2,
#                                                            merge_corr_thr=merge_corr_thr,merge_overlap_thr=merge_overlap_thr, num_plane=num_plane)
#        c_fin_tf = [];
#        if TF:
#            c_fin_tf = c_fin.copy();
...
#
#        return {'a':a, 'c':c, 'b':b, 'corr_img_all_r':corr_img_all_r, 'a_fin':a_fin, 'c_fin':c_fin, 'c_fin_tf':c_fin_tf, 'b_fin':b_fin, 'corr_img_all_r_fin':corr_img_all_r_fin, 'connect_mat_1':connect_mat_1_res, 'pure_pix':pure_pix_res, 'Cnt':Cnt_res, 'brightness_rank':brightness_rank_res}
#    else:
#        return {'a':a, 'c':c, 'b':b, 'corr_img_all_r':corr_img_all_r, 'connect_mat_1':connect_mat_1, 'pure_pix':pure_pix, 'Cnt':Cnt, 'brightness_rank':brightness_rank}
